...
then(data=>{
    ...
    data.items.forEach(category => {
        ...
        // Que hace? se crea un elemento table para cada categoria .Ademas, se prepara el encabezado(tableHead) con las columnas pertinentes. tableBody se inicializa vacio y se llenara con los elemtos  de la categoria.
        ...
        //Crear una table para los elementos de esas categorias: se genera una tabla por cada por cada categoria (primero se define el encabezado de la tabla <th>Foto</th><th>Description</th><th>Precio</th>).

        category.items.forEach (item=>{
            tableBody += `<tr>
            <td><imag src= ${item.image} alt=" ${item.image}" </td>
